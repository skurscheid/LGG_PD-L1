...
TCGA data for LGG and GBM samples has been downloaded between November 2015 and January 2016, and has been pre-processed to produce single data frames of expression valus for each dataset. Additionally, Pierre Bady ([link]pierre.bady@unil.ch) has compiled information about somatic mutations (incl. IDH1/2), CNV status, and CIMP status for the LGG dataset and kindly made these available to allow stratification of LGG samples.
...
# TCGA Level 3 RNA-Seq gene-level data was used, which is stored as FPKM (estimated fragments, normalized for transcript length)
# this data has been log2 transformed with an offset of 1 to avoid -Inf and approximate normal distribution of the data
...
# LGG samples
ncol(lgg.rna_seq.norm.log2)

# GBM samples
ncol(gbm.rna_seq.norm.log2)
```
...
**These esults show that we observe statistically significant difference in PD-L1 expression levels between the three LGG sub-groups (IDH^mut^ & 1p19q co-del, IDH^mut^ & 1p19q normal, IDH^wt^), while there is no difference between LGG IDH^wt^ and GBM samples.***
